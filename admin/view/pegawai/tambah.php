...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        $cek = mysqli_query($conn, "SELECT id_pegawai FROM pegawai WHERE username = '$username'");
        if (mysqli_num_rows($cek) > 0) $error[] = 'Username sudah digunakan.';
    }

    if (empty($error)) {
        // Generate ID: PGW001, PGW002, dst
        $last = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id_pegawai FROM pegawai ORDER BY id_pegawai DESC LIMIT 1"));
        $num  = $last ? (int) substr($last['id_pegawai'], 3) + 1 : 1;
        $id_pegawai = 'PGW' . str_pad($num, 3, '0', STR_PAD_LEFT);

        $pass = password_hash($password, PASSWORD_DEFAULT);
        mysqli_query($conn, "INSERT INTO pegawai (id_pegawai, nama_lengkap, username, password, no_telp, alamat, peran)
            VALUES ('$id_pegawai', '$nama_lengkap', '$username', '$pass', '$no_telp', '$alamat', '$peran')");
...
<div class="main-content">
    <div class="topbar">
        <h5><i class="bi bi-person-plus me-2"></i>Tambah Pegawai</h5>
        <span class="text-muted" style="font-size:.85rem;">Admin &rsaquo; Data Pegawai &rsaquo; Tambah</span>
    </div>

    <div class="card" style="max-width:600px">
        <div class="card-header py-3"><i class="bi bi-person-badge me-1"></i> Form Tambah Pegawai</div>
            ...
            <form method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="peran" class="form-select">
                        <option value="petugas_lapangan" <?= (($_POST['peran'] ?? '') === 'petugas_lapangan') ? 'selected' : '' ?>>Petugas Lapangan</option>
