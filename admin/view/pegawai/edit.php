...
$res = mysqli_query($conn, "SELECT * FROM pegawai WHERE id_pegawai = '$id'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        $cek = mysqli_query($conn, "SELECT id_pegawai FROM pegawai WHERE username = '$username' AND id_pegawai != '$id'");
        if (mysqli_num_rows($cek) > 0) $error[] = 'Username sudah digunakan pegawai lain.';
    }

    if (empty($error)) {
        if (!empty($password_baru)) {
            $pass = password_hash($password_baru, PASSWORD_DEFAULT);
            mysqli_query($conn, "UPDATE pegawai SET nama_lengkap='$nama_lengkap', username='$username', password='$pass', no_telp='$no_telp', alamat='$alamat', peran='$peran' WHERE id_pegawai='$id'");
        } else {
            mysqli_query($conn, "UPDATE pegawai SET nama_lengkap='$nama_lengkap', username='$username', no_telp='$no_telp', alamat='$alamat', peran='$peran' WHERE id_pegawai='$id'");
        }
...
<div class="main-content">
    <div class="topbar">
        <h5><i class="bi bi-person-gear me-2"></i>Edit Pegawai</h5>
        <span class="text-muted" style="font-size:.85rem;">Admin &rsaquo; Data Pegawai &rsaquo; Edit</span>
    </div>

    <div class="card" style="max-width:600px">
        <div class="card-header py-3"><i class="bi bi-person-badge me-1"></i> Form Edit Pegawai</div>
        <div class="card-body p-4">
            ...
            <div class="mb-3">
                <label class="form-label fw-semibold">ID Pegawai</label>
                <input type="text" class="form-control" value="<?= $row['id_pegawai'] ?>" disabled>
            </div>
            ...
            <form method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="peran" class="form-select">
                        <option value="petugas_lapangan" <?= ($row['peran'] === 'petugas_lapangan') ? 'selected' : '' ?>>Petugas Lapangan</option>
